...
export class BarchartComponent implements OnChanges {
  ...
  chart() {
    ...
    this.step = (400-this.yAxisPos)/(7*this.rectangles.length)
    //Finds biggestValue
    this.biggestValue =0
    ...
    for (let j = 0; j < this.rectangles.length; j++) {
      ...
      this.rectangles[j].x2 = this.rectangles[j].x1 + this.step*2.5;
      this.rectangles[j].usedValue = this.rectangles[j].value*this.xAxisPos/this.biggestValue
